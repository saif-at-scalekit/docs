...
<Note>
The auth proxy currently supports the [Playground](/langsmith/prompt-engineering-concepts), [Evals](/langsmith/evaluation), [Fleet](/langsmith/fleet), [Assistant](/langsmith/polly), and [Insights](/langsmith/insights) features.
...
To enable HTTP traffic to the auth proxy for [Assistant and Insights](/langsmith/deploy-self-hosted-full-platform#enable-fleet-insights-and-assistant), set this environment variable in the respective `extraEnv` sections: `config.polly.agent.extraEnv` and `config.insights.agent.extraEnv`.
</Accordion>
