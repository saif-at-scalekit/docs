...
<Note>
The auth proxy currently supports the [Playground](/langsmith/prompt-engineering-concepts), [Evals](/langsmith/evaluation), [Fleet](/langsmith/fleet), [Assistant](/langsmith/assistant), and [Insights](/langsmith/insights) features.
Playground and Evals are available in v0.13.33+. Assistant and Insights are available in v0.13.39+.
</Note>
...
To enable HTTP traffic to the auth proxy for [Assistant and Insights](/langsmith/deploy-self-hosted-full-platform#enable-fleet-insights-and-assistant), set this environment variable in the respective `extraEnv` sections: `config.polly.agent.extraEnv` (for Assistant which was formerly called Polly) and `config.insights.agent.extraEnv`.
</Accordion>
