...
1. In the LangSmith UI, navigate to **Annotation Queues** in the left sidebar. Open a single-run queue and select a run.
...
Click **Add to Dataset & Next** in the side panel footer (keyboard shortcut: <kbd>⌘ Enter</kbd> on macOS or <kbd>Ctrl Enter</kbd> elsewhere). LangSmith adds the current run to the queue's [default dataset](/langsmith/annotation-queues#basic-details), or prompts you to pick one if no default is configured. The queue then moves you to the next run.

The saved example's `outputs` field has this shape (YAML shown for readability; LangSmith stores it as JSON):
